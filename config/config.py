...
class Config:
    ...
    def __init__(self):
        ...
        self.media_dir = self.__get_str(config, plugin_config, 'GENERAL', 'media_dir', 'media')
        self.data_dir = self.__get_str(config, plugin_config, 'GENERAL', 'data_dir', 'data')
        self.log_dir = self.__get_str(config, plugin_config, 'GENERAL', 'log_dir', 'logs')
        ...
        self.log_file_name = self.__get_str(config, plugin_config, 'GENERAL', 'log_file_name', 'speed-cam.log')

        self.gui_window_on = self.__get_boolean(config, plugin_config, 'GUI', 'gui_window_on', False)
        self.show_thresh_on = self.__get_boolean(config, plugin_config, 'GUI', 'show_thresh_on', False)
        self.show_crop_on = self.__get_boolean(config, plugin_config, 'GUI', 'show_crop_on', False)

        self.SPEED_MPH = self.__get_boolean(config, plugin_config, 'TRACKING', 'SPEED_MPH', False)
        self.track_counter = self.__get_int(config, plugin_config, 'TRACKING', 'track_counter', 5)
        self.MIN_AREA = self.__get_int(config, plugin_config, 'TRACKING', 'MIN_AREA', 100)
        self.track_len_trig = self.__get_int(config, plugin_config, 'TRACKING', 'track_len_trig', 70)
        self.show_out_range = self.__get_boolean(config, plugin_config, 'TRACKING', 'show_out_range', True)
        self.x_diff_max = self.__get_int(config, plugin_config, 'TRACKING', 'x_diff_max', 20)
        self.x_diff_min = self.__get_int(config, plugin_config, 'TRACKING', 'x_diff_min', 1)
        self.x_buf_adjust = self.__get_int(config, plugin_config, 'TRACKING', 'x_buf_adjust', 10)
        self.track_timeout = self.__get_float(config, plugin_config, 'TRACKING', 'track_timeout', 0.0)
        self.event_timeout = self.__get_float(config, plugin_config, 'TRACKING', 'event_timeout', 0.3)
        self.max_speed_over = self.__get_int(config, plugin_config, 'TRACKING', 'max_speed_over', 0)
        ...
        self.image_path = self.__get_str(config, plugin_config, 'CAMERA', 'image_path', 'images')
        self.image_prefix = self.__get_str(config, plugin_config, 'CAMERA', 'image_prefix', 'speed-')
        self.image_format = config_validation.clean_file_ext_names(self.__get_str(config, plugin_config, 'CAMERA', 'image_format', '.jpg'))
        ...
        self.imageRecentDir = self.__get_str(config, plugin_config, 'CAMERA', 'imageRecentDir', 'recent')
        self.spaceTimerHrs = self.__get_int(config, plugin_config, 'CAMERA', 'spaceTimerHrs', 0)
        self.spaceFreeMB = config_validation.enforce_lower_bound_int(
            self.__get_int(config, plugin_config, 'CAMERA', 'spaceFreeMB', 500), 100)
        self.spaceFileExt = self.__get_str(config, plugin_config, 'CAMERA', 'spaceFileExt', 'jpg')
        ...
        self.html_path = self.__get_str(config, plugin_config, 'SERVER', 'html_path', 'html')
        ...
        self.search_source_images_path = self.__get_str(config, plugin_config, 'SEARCH', 'search_source_images_path', None)
    ...
    def display_config_verbose(self):
        ...
        if self.verbose:
            ...
            logging.info("                  loggingToFile=%s  logFilePath=%s", self.loggingToFile, self.log_file_name)
